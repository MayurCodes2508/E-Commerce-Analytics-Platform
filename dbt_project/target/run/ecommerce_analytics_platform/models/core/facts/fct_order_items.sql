-- back compat for old kwarg name
    ...
    merge into `intense-pixel-490219-h2`.`dev_core`.`fct_order_items` as DBT_INTERNAL_DEST
        ...
        * from `intense-pixel-490219-h2`.`dev_core`.`fct_order_items__dbt_tmp`
        ...
        on (
            (
                `DBT_INTERNAL_DEST`.`created_at` is null
                or `DBT_INTERNAL_DEST`.`created_at` is not null
            )
        ) and ((DBT_INTERNAL_SOURCE.order_item_key = DBT_INTERNAL_DEST.order_item_key))
