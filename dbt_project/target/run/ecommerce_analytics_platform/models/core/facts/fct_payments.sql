-- back compat for old kwarg name
    ...
    merge into `intense-pixel-490219-h2`.`dev_core`.`fct_payments` as DBT_INTERNAL_DEST
        ...
        * from `intense-pixel-490219-h2`.`dev_core`.`fct_payments__dbt_tmp`
        ...
        on (
            (
                `DBT_INTERNAL_DEST`.`payment_timestamp` is null
                or `DBT_INTERNAL_DEST`.`payment_timestamp` is not null
            )
        ) and ((DBT_INTERNAL_SOURCE.payment_key = DBT_INTERNAL_DEST.payment_key))
